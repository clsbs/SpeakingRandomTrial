...
if (/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent)
  ...
  document.getElementById('AudioCardBox').style.display = true;

  document.getElementById('divRandom').style.fontSize = "100px";
  document.getElementById('divRandom2').style.fontSize = "80px";
}
...
function getJetPageNumber(pageNumber) {
  let z = '<audio controls><source src="../src/' + pageNumber + '.m4a" type="audio/mp3"></audio>';
  document.getElementById('audioSetting').innerHTML = z;
  let y = '<img src="../images/grammar_image/glPage/jet34ks' + pageNumber + '.png' + '" alt="GrammarPage">';
  document.getElementById('glAudioCard').innerHTML = y;
};
...
function changeFontSize1() {
  // 1. 대상 element 선택
  const element = document.getElementById('divRandom');
  const element2 = document.getElementById('divRandom2');
  // 2. style 변경
...
const VERB_GENERAL_NEGATIVE_ALL = new Array("사랑하지 않는다 ", "좋아하지 않는다 ", "즐기지 않는다 ", "시작하지 않는다 ", "멈추지 않는다", "열지 않는다", "닫지 않는다", "요리하지 않는다", "먹지 않는다", "안 원한다", "안 달린다", "안 난다", "전화 안한다", "안 만든다", "안 준다", "안 간다", "안 온다", "말하지 않는다", "안 만난다", "안 떠난다", "가지고 있지 않다", "마시지 않는다", "점프하지 않는다", "잡지 않는다", "안 필요하다", "씻지 않는다", "걷지 않는다", "일어서지 않는다");
const VERB_GENERAL_INTERROGATIVE_ALL = new Array("사랑하니?", "좋아하니?", "즐기니?", "시작하니?", "멈추니?", "여니?", "닫니?", "요리하니?", "먹니?", "원하니?", "달리니?", "나니?", "전화하니?", "만드니?", "주니?", "가니?", "오니?", "말하니?", "만나니?", "떠나니?", "가지고 있니?", "마시니?", "뛰니?", "잡니?", "필요하니?", "씻니?", "걷니?", "일어서니?");
const VERB_GENERAL_ALL = new Array("사랑한다", "좋아한다", "즐긴다", "시작한다", "멈춘다", "연다", "닫는다", "요리한다", "먹는다", "원한다", "달린다", "난다", "전화한다", "만든다", "준다", "간다", "온다", "말한다", "만난다", "떠난다", "가지고 있다", "마신다", "점프한다", "잡는다", "필요하다", "씻는다", "걷는다", "일어선다", "앉는다", "넘어진다", "논다", "공부한다", "돕는다", "만진다", "생각한다", "당긴다", "민다", "그린다", "가져온다", "보낸다", "자른다", "읽는다", "움직인다", "묻는다", "사용한다", "한다", "들린다", "운다", "수영한다", "잔다", "사랑하지 않는다 ", "좋아하지 않는다 ", "즐기지 않는다 ", "시작하지 않는다 ", "멈추지 않는다", "열지 않는다", "닫지 않는다", "요리하지 않는다", "먹지 않는다", "안 원한다", "안 달린다", "안 난다", "전화 안한다", "안 만든다", "안 준다", "안 간다", "안 온다", "말하지 않는다", "안 만난다", "안 떠난다", "가지고 있지 않다", "마시지 않는다", "점프하지 않는다", "잡지 않는다", "안 필요하다", "씻지 않는다", "걷지 않는다", "일어서지 않는다", "사랑하니?", "좋아하니?", "즐기니?", "시작하니?", "멈추니?", "여니?", "닫니?", "요리하니?", "먹니?", "원하니?", "달리니?", "나니?", "전화하니?", "만드니?", "주니?", "가니?", "오니?", "말하니?", "만나니?", "떠나니?", "가지고 있니?", "마시니?", "뛰니?", "잡니?", "필요하니?", "씻니?", "걷니?", "일어서니?");

const VERB_GENERAL_PAST_ALL_K = new Array("사랑했다", "좋아했다", "즐겼다", "시작했다", "멈추었다", "열었다", "닫았다", "요리했다", "먹었다", "원했다", "달렸다", "났다", "전화했다", "만들었다", "줬다", "갔다", "왔다", "말했다", "만났다", "떠났다", "가지고 있었다", "마셨다", "점프했다", "잡았다", "필요했다", "씻었다", "걸었다", "일어섰다", "앉았다", "넘어졌다", "놀았다", "공부했다", "도왔다", "만질렀다", "생각했다", "당겼다", "민다", "그렸다", "가져왔다", "보냈다", "잘랐다", "읽었다", "움직였다", "물었다", "사용했다", "했다", "들렸다", "울었다", "수영했다", "잤다")
...
const noun1personK = new Array("성인", "천사", "이모", "아기", "상사", "소년", "형제", "선장", "아이", "직원", "부부", "사촌", "고객", "아빠", "딸", "치과의사", "의사", "기술자", "가족", "아버지", "바보", "친구", "신사", "소녀", "할아버지", "사람", "영웅", "인간", "남편", "아이", "왕", "여성", "어머니", "간호사", "부모", "사람들", "경찰", "왕자", "여왕", "그녀", "여동생", "아들", "학생", "10대의", "삼촌", "우리", "아내", "여성", "여러분", "농부", "외국인", "여걸", "소방관", "충고하는 사람", "10대 소년", "여행자", "동료", "학급친구", "한 방 사람", "인간", "육군 대장", "거지", "승객", "시민", "어릿광대", "상인", "거짓말쟁이", "임자", "적", "지배자", "도둑", "방문자", "여행자", "단짝", "교수", "고용주", "피고용인", "음악가", "배우", "여배우", "하인", "지도자", "해녀", "계산원", "뱃사람", "사냥꾼", "경영자", "변호사", "시장", "고객", "이발사", "남자", "여성", "운동선수", "지원자", "악마", "요정", "반대자", "청중", "탐험가", "백만장자", "기어오르는 사람", "동무", "주인", "이야기하는 사람", "상인", "지휘자", "시민", "일반인", "목수", "교대자", "선조", "범죄자", "사람들", "비서", "유권자", "우두머리", "조카", "조카딸", "시인", "강도", "겁쟁이", "교사", "디저트", "노예", "해군", "벌목꾼", "불교도", "소비자", "저자", "의사", "정치가", "천재", "살인자", "작곡가", "해군대장", "황제", "공주");
...
//TODO 동사 자료 넣기
const VERB_BASIC100_K = new Array();
const VERB_BASIC100_E = new Array();
const VERB_Basic_K = new Array("굽는다", "시작한다", "막는다", "깨뜨린다", "가져온다", "닦는다", "만든다", "점프한다", "산다", "전화한다", "보살핀다", "나른다", "잡는다", "점검한다", "확인한다", "선택한다", "오른다", "모은다", "된다", "축하한다", "제어한다", "요리한다", "복사한다", "덮는다", "건넌다", "오른다", "자른다", "춤춘다", "결정한다", "설계한다", "죽는다", "논의한다", "나눈다", "한다", "그린다", "꿈꾼다", "마신다", "운전한다", "떨어진다", "먹는다", "끝낸다", "즐긴다", "입장한다", "운동한다", "실패한다", "떨어진다", "느낀다", "싸운다", "채운다", "찾는다", "마친다", "고친다", "고정한다", "난다", "집중한다", "잊는다", "형성한다", "튀긴다", "받는다", "준다", "간다", "성장한다", "추측한다", "안내한다", "건다", "싫어한다", "가진다", "돕는다", "친다", "버틴다", "잡는다", "희망한다", "사냥한다", "서두른다", "소개한다", "초대한다", "참여한다", "유지한다", "찬다", "죽인다", "안다", "배운다", "거짓말한다", "좋아한다", "듣는다", "산다", "본다", "사랑한다", "만든다", "결혼한다", "만난다", "놓친다", "움직인다", "필요로 한다", "연다", "그린다", "통과한다", "지불한다", "선택한다", "계획한다", "논다", "인쇄한다", "민다", "놓는다", "읽는다", "기억한다", "돌아간다", "달린다", "구한다", "말한다", "본다", "판다", "보낸다", "놀라게 한다", "보여준다", "노래한다", "앉는다", "잔다", "냄새를 맡는다", "웃는다", "말한다", "일어선다", "시작한다", "머무른다", "멈춘다", "공부한다", "수영한다", "걸린다", "말한다", "가르친다", "말한다", "감사하다", "생각한다", "만진다", "훈련한다", "여행한다", "노력한다", "돌린다", "이해한다", "이용한다", "방문한다", "기다린다", "깬다", "걷는다", "원한다", "씻는다", "본다", "입는다", "환영한다", "우승한다", "바란다", "일한다", "걱정한다", "쓴다", "울린다", "가라앉는다", "뿌린다", "먹인다", "쏜다", "이끈다", "긴다", "쓴다", "운다", "떠난다", "구부린다", "빌린다", "잃는다", "소비한다", "의미한다", "찾는다", "묶는다", "감는다", "둔다", "듣는다", "미끄러진다", "판다", "빛난다", "돈다", "때린다", "때린다", "깨운다", "찢는다", "감춘다", "부른다", "던진다", "탄다", "오른다", "흔든다");
...
const VERB_ALL_PAST_E = new Array("acted", "added", "advise", "agreed", "answered", "arrived", "asked", "baked", "was, were", "became", "began", "believed", "bit", "blocked", "borrowed", "broke", "brought", "brushed", "built", "burned", "bought", "called", "cared", "carried", "caught", "changed", "checked", "chose", "climbed", "collected", "came", "congratulated", "controled", "cooked", "copied", "covered", "crossed", "cried", "cut", "danced", "decided", "designed", "died", "discussed", "divided", "did", "drew", "dreamed", "drank", "drove", "dropped", "ate", "ended", "enjoyed", "entered", "exercised", "failed", "fell", "felt", "fought", "filled", "found", "finished", "fixed", "fixed", "flew", "focused", "forgot", "formed", "fried", "got", "gave", "went", "grew", "guessed", "guided", "hung", "hated", "had", "helped", "hit", "held", "held", "hoped", "hunted", "hurried", "introduced", "invited", "joined", "kept", "kicked", "killed", "knew", "learned", "lied", "liked", "listened", "lived", "looked", "loved", "made", "married", "met", "missed", "moved", "needed", "opened", "painted", "passed", "paid", "picked", "planned", "played", "printed", "pushed", "put", "read", "remembered", "returned", "ran", "saved", "said", "saw", "sold", "sent", "shocked", "showed", "sang", "sat", "slept", "smelled", "smiled", "spoke", "stood", "started", "stayed", "stopped", "studied", "swam", "took", "talked", "taught", "told", "thanked", "tought", "touched", "trained", "traveled", "tried", "turned", "understood", "used", "visited", "waited", "woke", "walked", "wanted", "washed", "watched", "wore", "welcomed", "won", "wished", "worked", "worried", "wrote", "rang", "sank", "sowed", "fed", "shot", "led", "crept", "swept", "wept", "left", "bent", "lent", "lost", "spent", "meant", "sought", "bound", "wound", "laid", "heard", "slid", "dug", "shone", "spun", "struck", "beat", "awoke", "bore", "tore", "hid", "froze", "stole", "blew", "threw", "arose", "rode", "rose", "shook");
...
const TENSE_PAST_K = new Array("행동했다", "더했다", "조언했다", "동의했다", "대답했다", "도착했다", "물었다", "구었다", "이었다", "되었다", "시작했다", "믿었다", "물었다", "막았다", "빌렸다", "깼다", "가져왔다", "닦았다", "만들었다", "탔다", "샀다", "전화했다", "보살폈다", "날랐다", "잡았다", "바꿨다", "확인했다", "선택했다", "올랐다", "모았다", "왔다", "축하했다", "제어했다", "요리했다", "복사했다", "덮었다", "건넜다", "울었다", "잘랐다", "춤췄다", "결정했다", "설계했다", "죽었다", "논의했다", "나누었다", "했다", "그렸다", "꿈꿨다", "마셨다", "운전했다", "떨어졌다", "먹었다", "끝냈다", "즐겼다", "입장했다", "운동했다", "실패했다", "떨어졌다", "느꼈다", "싸웠다", "채웠다", "찾았다", "마쳤다", "고쳤다", "고정했다", "날았다", "집중했다", "잊었다", "형성했다", "튀겼다", "받았다", "전했다", "갔다", "성장했다", "추측했다", "안내했다", "걸었다", "싫어했다", "가졌다", "도왔다", "쳤다", "버텼다", "잡았다", "희망했다", "사냥했다", "서둘렀다", "소개했다", "초대했다", "참여했다", "유지했다", "찼다", "죽였다", "알았다", "배웠다", "거짓말했다", "좋아했다", "들었다", "살았다", "봤다", "사랑했다", "만들었다", "결혼했다", "만났다", "놓았다", "움직였다", "필요로 했다", "열었다", "그렸다", "통과했다", "지불했다", "선택했다", "계획했다", "놀았다", "인쇄했다", "밀었다", "넣었다", "읽었다", "기억했다", "돌아왔다", "달렸다", "구했다", "말했다", "봤다", "팔았다", "보냈다", "놀라게 했다", "보여줬다", "노래했다", "앉았다", "잤다", "냄새를 맡았다", "웃었다", "말했다", "일어섰다", "시작했다", "머물렀다", "멈췄다", "공부했다", "수영했다", "걸렸다", "말했다", "가르쳤다", "말했다", "감사했다", "생각했다", "만졌다", "훈련했다", "여행했다", "노력했다", "돌렸다", "이해했다", "이용했다", "방문했다", "기다렸다", "깼다", "걸었다", "원했다", "씻었다", "봤다(관람했다)", "입었다", "환영했다", "우승했다", "바랐다", "일했다", "걱정했다", "썼다", "울었다", "가라앉았다", "뿌렸다", "먹였다", "쐈다", "이끌었다", "기었다", "쓸었다", "울었다", "떠났다", "구부렸다", "빌었다", "잃었다", "소비했다", "의미했다", "찾았다", "묶었다", "감았다", "두었다(l)", "들었다", "미그러졌다", "팠다", "빛났다", "돌았다", "때렸다(s)", "때렸다(b)", "깨웠다", "낳았다", "찢었다", "감췄다", "얼렸다", "훔쳤다", "불었다", "던졌다", "일어났다", "탔다", "올랐다", "흔들었다");
...
const TENSE_CONTINUOUS_K = new Array("행동하고 있다", "더하고 있다", "조언하고 있다", "동의하고 있다", "대답하고 있다", "도착하고 있다", "묻고 있다", "굽고 있다", "이고 있다", "되고 있다", "시작하고 있다", "믿고 있다", "물고 있다", "막고 있다", "빌리고 있다", "깨뜨리고 있다", "가져오고 있다", "닦고 있다", "만들고 있다", "타고 있다", "사고 있다", "전화하고 있다", "보살피고 있다", "휴대하고 있다", "잡고 있다", "점검하고 있다", "확인하고 있다", "선택하고 있다", "오르고 있다", "모으고 있다", "오고 있다", "축하하고 있다", "제어하고 있다", "요리하고 있다", "복사하고 있다", "덮고 있다", "건너고 있다", "울고 있다", "자르고 있다", "춤추고 있다", "결정하고 있다", "설계하고 있다", "죽고 있다", "논의하고 있다", "나누고 있다", "하고 있다", "그리고 있다", "꿈꾸고 있다", "마시고 있다", "운전하고 있다", "떨어지고 있다", "먹고 있다", "끝내고 있다", "즐기고 있다", "입장하고 있다", "운동하고 있다", "실패하고 있다", "떨어지고 있다", "느끼고 있다", "싸우고 있다", "채우고 있다", "찾고 있다", "마치고 있다", "고치고 있다", "고정하고 있다", "날고 있다", "집중하고 있다", "잊고 있다", "형성하고 있다", "튀기고 있다", "받고 있다", "전하고 있다", "가고 있다", "성장하고 있다", "추측하고 있다", "안내하고 있다", "걸고 있다", "싫어하고 있다", "가지고 있다", "돕고 있다", "치고 있다", "버티고 있다", "잡고 있다", "희망하고 있다", "사냥하고 있다", "서두하고 있다", "소개하고 있다", "초대하고 있다", "참여하고 있다", "유지하고 있다", "치고 있다", "죽이고 있다", "알고 있다", "배우고 있다", "거짓말하고 있다", "좋아하고 있다", "듣고 있다", "살고 있다", "보고 있다", "사랑하고 있다", "만들고 있다", "결혼하고 있다", "만나고 있다", "놓고 있다", "움직이고 있다", "필요하고 있다", "열고 있다", "그리고 있다", "통과하고 있다", "지불하고 있다", "선택하고 있다", "계획하고 있다", "놀고 있다", "인쇄하고 있다", "밀고 있다", "넣고 있다", "읽고 있다", "기억하고 있다", "돌아가고 있다", "달리고 있다", "절약하고 있다", "말하고 있다", "보고 있다", "팔고 있다", "보내고 있다", "놀라하고 있다", "보여주고 있다", "노래하고 있다", "앉고 있다", "자고 있다", "냄새맡고 있다", "웃고 있다", "말하고 있다", "세우고 있다", "시작하고 있다", "머무르고 있다", "멈추고 있다", "공부하고 있다", "수영하고 있다", "걸리고 있다", "말하고 있다", "가르치고 있다", "말하고 있다", "말하고 있다", "생각하고 있다", "만지고 있다", "훈련하고 있다", "여행하고 있다", "노력하고 있다", "돌리고 있다", "이해하고 있다", "이용하고 있다", "방문하고 있다", "기다리고 있다", "깨고 있다", "걷고 있다", "원하고 있다", "씻고 있다", "관람하고 있다", "입고 있다", "환영하고 있다", "우승하고 있다", "바라고 있다", "일하고 있다", "걱정하고 있다", "쓰고 있다", "울리고 있다", "가라앉고 있다", "뿌리고 있다", "먹이고 있다", "쏘고 있다", "이끌고 있다", "기고 있다", "쓸고 있다", "울고 있다", "떠나오고 있다", "구부리고 있다", "빌리고 있다", "잃고 있다", "소비하고 있다", "의미하고 있다", "찾고 있다", "묶고 있다", "감고 있다", "두고 있다", "듣고 있다", "미끄러지고 있다", "파고 있다", "빛나고 있다", "돌고 있다", "때리고 있다", "때리고 있다", "깨우고 있다", "낳고 있다", "찢고 있다", "감추고 있다", "얼고 있다", "훔치고 있다", "불고 있다", "던지고 있다", "일어나고 있다", "타고 있다", "오르고 있다", "흔들고 있다");
const TENSE_CONTINUOUS_Kv2 = new Array("행동하는 중이다", "더하고 있다", "조언하고 있다", "동의하고 있다", "대답하고 있다", "도착하고 있다", "묻고 있다", "굽고 있다", "이고 있다", "되고 있다", "시작하고 있다", "믿고 있다", "물고 있다", "막고 있다", "빌리고 있다", "깨뜨리고 있다", "가져오고 있다", "닦고 있다", "만들고 있다", "타고 있다", "사고 있다", "전화하고 있다", "보살피고 있다", "휴대하고 있다", "잡고 있다", "점검하고 있다", "확인하고 있다", "선택하고 있다", "오르고 있다", "모으고 있다", "오고 있다", "축하하고 있다", "제어하고 있다", "요리하고 있다", "복사하고 있다", "덮고 있다", "건너고 있다", "울고 있다", "자르고 있다", "춤추고 있다", "결정하고 있다", "설계하고 있다", "죽고 있다", "논의하고 있다", "나누고 있다", "하고 있다", "그리고 있다", "꿈꾸고 있다", "마시고 있다", "운전하고 있다", "떨어지고 있다", "먹고 있다", "끝내고 있다", "즐기고 있다", "입장하고 있다", "운동하고 있다", "실패하고 있다", "떨어지고 있다", "느끼고 있다", "싸우고 있다", "채우고 있다", "찾고 있다", "마치고 있다", "고치고 있다", "고정하고 있다", "날고 있다", "집중하고 있다", "잊고 있다", "형성하고 있다", "튀기고 있다", "받고 있다", "전하고 있다", "가고 있다", "성장하고 있다", "추측하고 있다", "안내하고 있다", "걸고 있다", "싫어하고 있다", "가지고 있다", "돕고 있다", "치고 있다", "버티고 있다", "잡고 있다", "희망하고 있다", "사냥하고 있다", "서두하고 있다", "소개하고 있다", "초대하고 있다", "참여하고 있다", "유지하고 있다", "치고 있다", "죽이고 있다", "알고 있다", "배우고 있다", "거짓말하고 있다", "좋아하고 있다", "듣고 있다", "살고 있다", "보고 있다", "사랑하고 있다", "만들고 있다", "결혼하고 있다", "만나고 있다", "놓고 있다", "움직이고 있다", "필요하고 있다", "열고 있다", "그리고 있다", "통과하고 있다", "지불하고 있다", "선택하고 있다", "계획하고 있다", "놀고 있다", "인쇄하고 있다", "밀고 있다", "넣고 있다", "읽고 있다", "기억하고 있다", "돌아가고 있다", "달리고 있다", "절약하고 있다", "말하고 있다", "보고 있다", "팔고 있다", "보내고 있다", "놀라하고 있다", "보여주고 있다", "노래하고 있다", "앉고 있다", "자고 있다", "냄새맡고 있다", "웃고 있다", "말하고 있다", "세우고 있다", "시작하고 있다", "머무르고 있다", "멈추고 있다", "공부하고 있다", "수영하고 있다", "걸리고 있다", "말하고 있다", "가르치고 있다", "말하고 있다", "말하고 있다", "생각하고 있다", "만지고 있다", "훈련하고 있다", "여행하고 있다", "노력하고 있다", "돌리고 있다", "이해하고 있다", "이용하고 있다", "방문하고 있다", "기다리고 있다", "깨고 있다", "걷고 있다", "원하고 있다", "씻고 있다", "관람하고 있다", "입고 있다", "환영하고 있다", "우승하고 있다", "바라고 있다", "일하고 있다", "걱정하고 있다", "쓰고 있다", "울리고 있다", "가라앉고 있다", "뿌리고 있다", "먹이고 있다", "쏘고 있다", "이끌고 있다", "기고 있다", "쓸고 있다", "울고 있다", "떠나오고 있다", "구부리고 있다", "빌리고 있다", "잃고 있다", "소비하고 있다", "의미하고 있다", "찾고 있다", "묶고 있다", "감고 있다", "두고 있다", "듣고 있다", "미끄러지고 있다", "파고 있다", "빛나고 있다", "돌고 있다", "때리고 있다", "때리고 있다", "깨우고 있다", "낳고 있다", "찢고 있다", "감추고 있다", "얼고 있다", "훔치고 있다", "불고 있다", "던지고 있다", "일어나고 있다", "타고 있다", "오르고 있다", "흔들고 있다");
const TENSE_CONTINUOUS_E = new Array("acting", "adding", "advising", "agreeing", "answering", "arriving", "asking", "baking", "being", "becoming", "beginning", "believing", "biting", "blocking", "borrowing", "breaking", "bringing", "brushing", "building", "burning", "buying", "calling", "caring", "carrying", "catching", "changing", "checking", "choosing", "climbing", "collecting", "coming", "congratulating", "controlling", "cooking", "copying", "covering", "crossing", "crying", "cutting", "dancing", "deciding", "designing", "dying", "discussing", "dividing", "doing", "drawing", "dreaming", "drinking", "driving", "dropping", "eating", "ending", "enjoying", "entering", "exercising", "failing", "falling", "feeling", "fighting", "filling", "finding", "finishing", "fixing", "fixing", "flying", "focusing", "forgetting", "forming", "frying", "getting", "giving", "going", "growing", "guessing", "guiding", "hanging", "hating", "having", "helping", "hitting", "holding", "holding", "hoping", "hunting", "hurrying", "introducing", "inviting", "joining", "keeping", "kicking", "killing", "knowing", "learning", "lying", "liking", "listening", "living", "looking", "loving", "making", "marrying", "meeting", "missing", "moving", "needing", "opening", "painting", "passing", "paying", "picking", "planning", "playing", "printing", "pusing", "putting", "reading", "remembering", "returning", "running", "saving", "saying", "seeing", "selling", "sending", "shocking", "showing", "singing", "sitting", "sleeping", "smelling", "smiling", "speaking", "standing", "starting", "staying", "stopping", "studying", "swimming", "taking", "talking", "teaching", "telling", "thanking", "thinking", "touching", "training", "traveling", "trying", "turning", "understanding", "using", "visiting", "waiting", "waking", "walking", "wanting", "washing", "watching", "wearing", "welcoming", "winning", "wishing", "working", "worrying", "writing", "ringing", "singing", "sowing", "feeding", "shooting", "leading", "creeping", "sweeping", "weeping", "leaving", "bending", "lending", "losing", "spending", "meaning", "seeking", "binding", "winding", "laying", "hearing", "sliding", "digging", "shining", "spinning", "striking", "beating", "awaking", "bearing", "tearing", "hiding", "freezing", "stealing", "blowing", "throwing", "arising", "riding", "rising", "shaking");
const TENSE_CONTINUOUS_PAST_K = new Array("행동하는 중이었다", "더하는 중이었다", "조언하는 중이었다", "동의하는 중이었다", "대답하는 중이었다", "도착하는 중이었다", "묻는 중이었다", "굽는 중이었다", "이는 중이었다", "되는 중이었다", "시작하는 중이었다", "믿는 중이었다", "무는 중이었다", "막는 중이었다", "빌리는 중이었다", "깨뜨리는 중이었다", "가져오는 중이었다", "닦는 중이었다", "만드는 중이었다", "타는 중이었다", "사는 중이었다", "전화하는 중이었다", "보살피는 중이었다", "휴대하는 중이었다", "잡는 중이었다", "바꾸는 중이었다", "확인하는 중이었다", "선택하는 중이었다", "오르는 중이었다", "모으는 중이었다", "오는 중이었다", "축하하는 중이었다", "제어하는 중이었다", "요리하는 중이었다", "복사하는 중이었다", "덮는 중이었다", "건너는 중이었다", "우는 중이었다", "자르는 중이었다", "춤추는 중이었다", "결정하는 중이었다", "설계하는 중이었다", "죽는 중이었다", "논의하는 중이었다", "나누는 중이었다", "하는 중이었다", "그리는 중이었다", "꿈꾸는 중이었다", "마시는 중이었다", "운전하는 중이었다", "떨어지는 중이었다", "먹는 중이었다", "끝내는 중이었다", "즐기는 중이었다", "입장하는 중이었다", "운동하는 중이었다", "실패하는 중이었다", "떨어지는 중이었다", "느끼는 중이었다", "싸우는 중이었다", "채우는 중이었다", "찾는 중이었다", "마치는 중이었다", "고치는 중이었다", "고정하는 중이었다", "나는 중이었다", "집중하는 중이었다", "잊는 중이었다", "형성하는 중이었다", "튀기는 중이었다", "받는 중이었다", "전하는 중이었다", "가는 중이었다", "성장하는 중이었다", "추측하는 중이었다", "안내하는 중이었다", "거는 중이었다", "싫어하는 중이었다", "가지는 중이었다", "돕는 중이었다", "치는 중이었다", "버티는 중이었다", "잡는 중이었다", "희망하는 중이었다", "사냥하는 중이었다", "서두하는 중이었다", "소개하는 중이었다", "초대하는 중이었다", "참여하는 중이었다", "유지하는 중이었다", "치는 중이었다", "죽이는 중이었다", "아는 중이었다", "배우는 중이었다", "거짓말하는 중이었다", "좋아하는 중이었다", "듣는 중이었다", "사는 중이었다", "보는 중이었다", "사랑하는 중이었다", "만드는 중이었다", "결혼하는 중이었다", "만나는 중이었다", "놓는 중이었다", "움직이는 중이었다", "필요하는 중이었다", "여는 중이었다", "그리는 중이었다", "통과하는 중이었다", "지불하는 중이었다", "선택하는 중이었다", "계획하는 중이었다", "노는 중이었다", "인쇄하는 중이었다", "미는 중이었다", "넣는 중이었다", "읽는 중이었다", "기억하는 중이었다", "돌아가는 중이었다", "달리는 중이었다", "절약하는 중이었다", "말하는 중이었다", "보는 중이었다", "파는 중이었다", "보내는 중이었다", "놀라하는 중이었다", "보여주는 중이었다", "노래하는 중이었다", "앉는 중이었다", "자는 중이었다", "냄새맡는 중이었다", "웃는 중이었다", "말하는 중이었다", "세우는 중이었다", "시작하는 중이었다", "머무르는 중이었다", "멈추는 중이었다", "공부하는 중이었다", "수영하는 중이었다", "걸리는 중이었다", "말하는 중이었다", "가르치는 중이었다", "말하는 중이었다", "말하는 중이었다", "생각하는 중이었다", "만지는 중이었다", "훈련하는 중이었다", "여행하는 중이었다", "노력하는 중이었다", "돌리는 중이었다", "이해하는 중이었다", "이용하는 중이었다", "방문하는 중이었다", "기다리는 중이었다", "깨는 중이었다", "걷는 중이었다", "원하는 중이었다", "씻는 중이었다", "관람하는 중이었다", "입는 중이었다", "환영하는 중이었다", "우승하는 중이었다", "바라는 중이었다", "일하는 중이었다", "걱정하는 중이었다", "쓰는 중이었다", "울리는 중이었다", "가라앉는 중이었다", "뿌리는 중이었다", "먹이는 중이었다", "쏘는 중이었다", "이끄는 중이었다", "기는 중이었다", "쓰는 중이었다", "우는 중이었다", "떠나오는 중이었다", "구부리는 중이었다", "빌리는 중이었다", "잃는 중이었다", "소비하는 중이었다", "의미하는 중이었다", "찾는 중이었다", "묶는 중이었다", "감는 중이었다", "두는 중이었다", "듣는 중이었다", "미끄러지는 중이었다", "파는 중이었다", "빛나는 중이었다", "도는 중이었다", "때리는 중이었다", "때리는 중이었다", "깨우는 중이었다", "낳는 중이었다", "찢는 중이었다", "감추는 중이었다", "어는 중이었다", "훔치는 중이었다", "부는 중이었다", "던지는 중이었다", "일어나는 중이었다", "타는 중이었다", "오르는 중이었다", "흔드는 중이었다");
const TENSE_CONTINUOUS_PAST_E = TENSE_CONTINUOUS_E;
const TENSE_CONTINUOUS_FUTURE_K = new Array("행동하는 중일 것이다", "더하는 중일 것이다", "조언하는 중일 것이다", "동의하는 중일 것이다", "대답하는 중일 것이다", "도착하는 중일 것이다", "묻는 중일 것이다", "굽는 중일 것이다", "이는 중일 것이다", "되는 중일 것이다", "시작하는 중일 것이다", "믿는 중일 것이다", "무는 중일 것이다", "막는 중일 것이다", "빌리는 중일 것이다", "깨뜨리는 중일 것이다", "가져오는 중일 것이다", "닦는 중일 것이다", "만드는 중일 것이다", "타는 중일 것이다", "사는 중일 것이다", "전화하는 중일 것이다", "보살피는 중일 것이다", "휴대하는 중일 것이다", "잡는 중일 것이다", "바꾸는 중일 것이다", "확인하는 중일 것이다", "선택하는 중일 것이다", "오르는 중일 것이다", "모으는 중일 것이다", "오는 중일 것이다", "축하하는 중일 것이다", "제어하는 중일 것이다", "요리하는 중일 것이다", "복사하는 중일 것이다", "덮는 중일 것이다", "건너는 중일 것이다", "우는 중일 것이다", "자르는 중일 것이다", "춤추는 중일 것이다", "결정하는 중일 것이다", "설계하는 중일 것이다", "죽는 중일 것이다", "논의하는 중일 것이다", "나누는 중일 것이다", "하는 중일 것이다", "그리는 중일 것이다", "꿈꾸는 중일 것이다", "마시는 중일 것이다", "운전하는 중일 것이다", "떨어지는 중일 것이다", "먹는 중일 것이다", "끝내는 중일 것이다", "즐기는 중일 것이다", "입장하는 중일 것이다", "운동하는 중일 것이다", "실패하는 중일 것이다", "떨어지는 중일 것이다", "느끼는 중일 것이다", "싸우는 중일 것이다", "채우는 중일 것이다", "찾는 중일 것이다", "마치는 중일 것이다", "고치는 중일 것이다", "고정하는 중일 것이다", "나는 중일 것이다", "집중하는 중일 것이다", "잊는 중일 것이다", "형성하는 중일 것이다", "튀기는 중일 것이다", "받는 중일 것이다", "전하는 중일 것이다", "가는 중일 것이다", "성장하는 중일 것이다", "추측하는 중일 것이다", "안내하는 중일 것이다", "거는 중일 것이다", "싫어하는 중일 것이다", "가지는 중일 것이다", "돕는 중일 것이다", "치는 중일 것이다", "버티는 중일 것이다", "잡는 중일 것이다", "희망하는 중일 것이다", "사냥하는 중일 것이다", "서두하는 중일 것이다", "소개하는 중일 것이다", "초대하는 중일 것이다", "참여하는 중일 것이다", "유지하는 중일 것이다", "치는 중일 것이다", "죽이는 중일 것이다", "아는 중일 것이다", "배우는 중일 것이다", "거짓말하는 중일 것이다", "좋아하는 중일 것이다", "듣는 중일 것이다", "사는 중일 것이다", "보는 중일 것이다", "사랑하는 중일 것이다", "만드는 중일 것이다", "결혼하는 중일 것이다", "만나는 중일 것이다", "놓는 중일 것이다", "움직이는 중일 것이다", "필요하는 중일 것이다", "여는 중일 것이다", "그리는 중일 것이다", "통과하는 중일 것이다", "지불하는 중일 것이다", "선택하는 중일 것이다", "계획하는 중일 것이다", "노는 중일 것이다", "인쇄하는 중일 것이다", "미는 중일 것이다", "넣는 중일 것이다", "읽는 중일 것이다", "기억하는 중일 것이다", "돌아가는 중일 것이다", "달리는 중일 것이다", "절약하는 중일 것이다", "말하는 중일 것이다", "보는 중일 것이다", "파는 중일 것이다", "보내는 중일 것이다", "놀라하는 중일 것이다", "보여주는 중일 것이다", "노래하는 중일 것이다", "앉는 중일 것이다", "자는 중일 것이다", "냄새맡는 중일 것이다", "웃는 중일 것이다", "말하는 중일 것이다", "세우는 중일 것이다", "시작하는 중일 것이다", "머무르는 중일 것이다", "멈추는 중일 것이다", "공부하는 중일 것이다", "수영하는 중일 것이다", "걸리는 중일 것이다", "말하는 중일 것이다", "가르치는 중일 것이다", "말하는 중일 것이다", "말하는 중일 것이다", "생각하는 중일 것이다", "만지는 중일 것이다", "훈련하는 중일 것이다", "여행하는 중일 것이다", "노력하는 중일 것이다", "돌리는 중일 것이다", "이해하는 중일 것이다", "이용하는 중일 것이다", "방문하는 중일 것이다", "기다리는 중일 것이다", "깨는 중일 것이다", "걷는 중일 것이다", "원하는 중일 것이다", "씻는 중일 것이다", "관람하는 중일 것이다", "입는 중일 것이다", "환영하는 중일 것이다", "우승하는 중일 것이다", "바라는 중일 것이다", "일하는 중일 것이다", "걱정하는 중일 것이다", "쓰는 중일 것이다", "울리는 중일 것이다", "가라앉는 중일 것이다", "뿌리는 중일 것이다", "먹이는 중일 것이다", "쏘는 중일 것이다", "이끄는 중일 것이다", "기는 중일 것이다", "쓰는 중일 것이다", "우는 중일 것이다", "떠나오는 중일 것이다", "구부리는 중일 것이다", "빌리는 중일 것이다", "잃는 중일 것이다", "소비하는 중일 것이다", "의미하는 중일 것이다", "찾는 중일 것이다", "묶는 중일 것이다", "감는 중일 것이다", "두는 중일 것이다", "듣는 중일 것이다", "미끄러지는 중일 것이다", "파는 중일 것이다", "빛나는 중일 것이다", "도는 중일 것이다", "때리는 중일 것이다", "때리는 중일 것이다", "깨우는 중일 것이다", "낳는 중일 것이다", "찢는 중일 것이다", "감추는 중일 것이다", "어는 중일 것이다", "훔치는 중일 것이다", "부는 중일 것이다", "던지는 중일 것이다", "일어나는 중일 것이다", "타는 중일 것이다", "오르는 중일 것이다", "흔드는 중일 것이다");
...
const PARTICIPLE_K = new Array("행동하는", "더하는", "조언하는", "동의하는", "대답하는", "도착하는", "요청하는", "굽는", "인", "되는", "시작하는", "믿는", "무는", "막는", "빌리는", "깨뜨리는", "가져오는", "닦는", "만드는", "타는", "사는", "전화하는", "보살피는", "휴대하는", "잡는", "점검하는", "확인하는", "선택하는", "오르는", "모으는", "하는", "축하하는", "제어하는", "요리하는", "복사하는", "덮는", "건너는", "우는", "자르는", "춤추는", "결정하는", "설계하는", "죽는", "논의하는", "나누는", "되는", "그리는", "꿈꾸는", "마시는", "운전하는", "떨어지는", "먹는", "끝내는", "즐기는", "입장하는", "훈련하는", "실패하는", "떨어지는", "느끼는", "싸우는", "채우는", "찾는", "마치는", "고치는", "고정하는", "나는", "집중하는", "잊는", "형성하는", "튀기는", "얻는", "주는", "가는", "자라는", "추측하는", "안내하는", "거는", "미워하는", "가지는", "돕는", "치는", "버티는", "잡는", "희망하는", "사냥하는", "서두르는", "소개하는", "초대하는", "참여하는", "유지하는", "차는", "죽이는", "아는", "배우는", "거짓말하는", "좋아하는", "듣는", "사는", "보는", "사랑하는", "만드는", "결혼하는", "만나는", "놓는", "움직이는", "필요한", "여는", "그리는", "통과하는", "지불하는", "선택하는", "계획하는", "노는", "인쇄하는", "미는", "놓는", "읽는", "기억하는", "되돌리는", "달리는", "절약하는", "말하는", "보는", "파는", "보내는", "놀라게 하는", "보여주는", "노래하는", "앉는", "자는", "냄새를 맡는", "웃는", "말하는", "일어서는", "시작하는", "머무르는", "멈추는", "공부하는", "수영하는", "걸리는", "말하는", "가르치는", "말하는", "고마워하는", "생각하는", "만지는", "훈련하는", "여행하는", "노력하는", "돌리는", "이해하는", "이용하는", "방문하는", "기다리는", "깨는", "걷는", "원하는", "씻는", "관람하는", "입는", "환영하는", "우승하는", "바라는", "일하는", "걱정하는", "쓰는", "울리는", "가라앉는", "뿌리는", "먹이는", "쏘는", "이끄는", "기는", "쓰는", "우는", "떠나는", "구부리는", "빌리는", "잃는", "소비하는", "의미하는", "찾는", "묶는", "감는", "두는", "듣는", "미끄러지는", "파는", "밝은", "도는", "때리는", "때리는", "깨는", "낳는", "찢는", "감추는", "얼리는", "훔치는", "부는", "던지는", "일어나는", "타는", "오르는", "흔드는");
const PARTICIPLE_E = new Array();
...
const PASSIVE_E = new Array("acted", "added", "advised", "agreed", "answered", "arrived", "asked", "baked", "been", "become", "begun", "believed", "bitten", "blocked", "borrowed", "broken", "brought", "brushed", "built", "burned", "bought", "called", "cared", "carried", "caught", "changed", "checked", "chosen", "climbed", "collected", "come", "congratulated", "controled", "cooked", "copied", "covered", "crossed", "cried", "cut", "danced", "decided", "designed", "died", "discussed", "divided", "done", "drawn", "dreamed", "drunk", "driven", "dropped", "eaten", "ended", "enjoyed", "entered", "exercised", "failed", "fallen", "felt", "fought", "filled", "found", "finished", "fixed", "fixed", "flown", "focused", "forgotten", "formed", "fried", "gotten", "given", "gone", "grown", "guessed", "guided", "hung", "hated", "had", "helped", "hit", "held", "held", "hoped", "hunted", "hurried", "introduced", "invited", "joined", "kept", "kicked", "killed", "known", "learned", "lied", "liked", "listened", "lived", "looked", "loved", "made", "married", "met", "missed", "moved", "needed", "opened", "painted", "passed", "paid", "picked", "planned", "played", "printed", "pushed", "put", "read", "remembered", "returned", "run", "saved", "said", "seen", "sold", "sent", "shocked", "showed", "sung", "sat", "slept", "smelled", "smiled", "spoken", "stood", "started", "stayed", "stopped", "studied", "swum", "taken", "talked", "taught", "told", "thanked", "thought", "touched", "trained", "traveled", "tried", "turned", "understood", "used", "visited", "waited", "woken", "walked", "wanted", "washed", "watched", "worn", "welcomed", "won", "wished", "worked", "worried", "written", "rung", "sunk", "sowed", "fed", "shot", "led", "crept", "swept", "wept", "left", "bent", "lent", "lost", "spent", "meant", "sought", "bound", "wound", "laid", "heard", "slid", "dug", "shone", "spun", "struck", "beaten", "awoken", "born", "torn", "hidden", "frozen", "stolen", "blown", "thrown", "arisen", "ridden", "risen", "shaken");
...
const PASSIVE_SENTENCE_2K = new Array("행동되었다", "더해졌다", "조언받았다", "동의되었다", "대답되었다", "도착되었다", "요청되었다", "구워졌다", "되었다", "되었다", "시작되었다", "믿어졌다", "물렸다", "막아졌다", "빌려졌다", "깨뜨려졌다", "가져와졌다", "닦아졌다", "만들어졌다", "타졌다", "사졌다", "전화되었다", "보살펴졌다", "휴대되었다", "잡아졌다", "점검되었다", "확인되었다", "선택되었다");
...
const PASSIVE_SENTENCE_PATTERN_K = new Array("1형식 SV - 수동태 가능?", "2형식 SVC - 수동태 가능?", "3형식 SVO - 수동태 가능?", "4형식 SVOO - 수동태 가능?", "5형식 SVOC - 수동태 가능?");
...
const VERB_NOT_PASSIVE_K = new Array("수동태 불가동사?", "수동태 불가동사1 - 자동사?", "수동태 불가동사2 - 상태/소유동사?");
const VERB_NOT_PASSIVE_E = new Array("자동사, 상태동사, 소유동사", "arrive, rise, happen, (dis)appear, look", "have, meet, resemble, belong to, fit, lack, cost 등");
...
const SUBJUNCTIVE_AUXILIARY_COULD_K = new Array("행동할 수 있을 텐데", "조언할 수 있을 텐데", "동의할 수 있을 텐데", "도착할 수 있을 텐데", "물어볼 수 있을 텐데", "구울 수 있을 텐데", "시작할 수 있을 텐데", "믿을 수 있을 텐데", "깨뜨릴 수 있을 텐데", "탈 수 있을 텐데", "보살필 수 있을 텐데", "바꿀 수 있을 텐데", "확인할 수 있을 텐데", "선택할 수 있을 텐데", "오를 수 있을 텐데", "올 수 있을 텐데", "요리할 수 있을 텐데", "복사할 수 있을 텐데", "건널 수 있을 텐데", "울 수 있을 텐데", "자를 수 있을 텐데", "춤출 수 있을 텐데", "결정할 수 있을 텐데", "죽을 수 있을 텐데", "할 수 있을 텐데", "그릴 수 있을 텐데", "꿈꿀 수 있을 텐데", "마실 수 있을 텐데", "운전할 수 있을 텐데", "떨어뜨릴 수 있을 텐데", "먹을 수 있을 텐데", "끝날 수 있을 텐데", "들어갈 수 있을 텐데", "운동할 수 있을 텐데", "실패할 수 있을 텐데", "떨어질 수 있을 텐데", "느낄 수 있을 텐데", "싸울 수 있을 텐데", "마칠 수 있을 텐데", "고정할 수 있을 텐데", "날 수 있을 텐데", "집중할 수 있을 텐데", "잊을 수 있을 텐데", "형성할 수 있을 텐데", "튀길 수 있을 텐데", "갈 수 있을 텐데", "자랄 수 있을 텐데", "추측할 수 있을 텐데", "걸을 수 있을 텐데", "도울 수 있을 텐데", "칠 수 있을 텐데", "잡을 수 있을 텐데", "희망할 수 있을 텐데", "서두를 수 있을 텐데", "참여할 수 있을 텐데", "계속할 수 있을 텐데", "찰 수 있을 텐데", "알 수 있을 텐데", "거짓말 할 수 있을 텐데", "들을 수 있을 텐데", "살 수 있을 텐데", "볼 수 있을 텐데", "사랑할 수 있을 텐데", "움직일 수 있을 텐데", "열 수 있을 텐데", "통과할 수 있을 텐데", "지불할 수 있을 텐데", "놀 수 있을 텐데", "밀할 수 있을 텐데", "읽을 수 있을 텐데", "돌아갈 수 있을 텐데", "달릴 수 있을 텐데", "노래할 수 있을 텐데", "앉을 수 있을 텐데", "잘 수 있을 텐데", "냄새날 수 있을 텐데", "미소지을 수 있을 텐데", "말할 수 있을 텐데", "일어설 수 있을 텐데", "시작할 수 있을 텐데", "머무를 수 있을 텐데", "멈출 수 있을 텐데", "수영할 수 있을 텐데", "말할 수 있을 텐데", "이야기할 수 있을 텐데", "여행할 수 있을 텐데", "노력할 수 있을 텐데", "돌릴 수 있을 텐데", "방문할 수 있을 텐데", "기다릴 수 있을 텐데", "깰 수 있을 텐데", "걸을 수 있을 텐데", "씻을 수 있을 텐데", "입을 수 있을 텐데", "이길 수 있을 텐데", "바랄 수 있을 텐데", "일할 수 있을 텐데", "걱정할 수 있을 텐데", "쓸 수 있을 텐데", "울릴 수 있을 텐데", "가라앉을 수 있을 텐데", "먹일 수 있을 텐데", "쏠 수 있을 텐데", "이끌 수 있을 텐데", "기다릴 수 있을 텐데", "바닥을 쓸 수 있을 텐데", "울 수 있을 텐데", "떠날 수 있을 텐데", "구부릴 수 있을 텐데", "잃을 수 있을 텐데", "감을 수 있을 텐데", "미끄러질 수 있을 텐데", "팔 수 있을 텐데", "빛날 수 있을 텐데", "돌 수 있을 텐데", "때릴 수 있을 텐데", "깨울 수 있을 텐데", "낳을 수 있을 텐데", "찢을 수 있을 텐데", "얼 수 있을 텐데", "훔칠 수 있을 텐데", "불 수 있을 텐데", "던질 수 있을 텐데", "탈 수 있을 텐데", "오를 수 있을 텐데", "흔들 수 있을 텐데");
const SUBJUNCTIVE_AUXILIARY_COULD_HAVE_K = new Array(
  "행동할 수 있었을 텐데", "조언할 수 있었을 텐데", "동의할 수 있었을 텐데", "도착할 수 있었을 텐데", "물어볼 수 있었을 텐데", "구울 수 있었을 텐데", "시작할 수 있었을 텐데", "믿을 수 있었을 텐데", "깨뜨릴 수 있었을 텐데", "탈 수 있었을 텐데", "보살필 수 있었을 텐데", "바꿀 수 있었을 텐데", "확인할 수 있었을 텐데", "선택할 수 있었을 텐데", "오를 수 있었을 텐데", "올 수 있었을 텐데", "요리할 수 있었을 텐데", "복사할 수 있었을 텐데", "건널 수 있었을 텐데", "울 수 있었을 텐데", "자를 수 있었을 텐데", "춤출 수 있었을 텐데", "결정할 수 있었을 텐데", "죽을 수 있었을 텐데", "할 수 있었을 텐데", "그릴 수 있었을 텐데", "꿈꿀 수 있었을 텐데", "마실 수 있었을 텐데", "운전할 수 있었을 텐데", "떨어뜨릴 수 있었을 텐데", "먹을 수 있었을 텐데", "끝낼 수 있었을 텐데", "들어갈 수 있었을 텐데", "운동할 수 있었을 텐데", "실패할 수 있었을 텐데", "떨어질 수 있었을 텐데", "느낄 수 있었을 텐데", "싸울 수 있었을 텐데", "마칠 수 있었을 텐데", "고정할 수 있었을 텐데", "날 수 있었을 텐데", "집중할 수 있었을 텐데", "잊을 수 있었을 텐데", "형성할 수 있었을 텐데", "튀길 수 있었을 텐데", "갈 수 있었을 텐데", "자랄 수 있었을 텐데", "추측할 수 있었을 텐데", "걸을 수 있었을 텐데", "도울 수 있었을 텐데", "칠 수 있었을 텐데", "잡을 수 있었을 텐데", "희망할 수 있었을 텐데", "서두를 수 있었을 텐데", "참여할 수 있었을 텐데", "계속할 수 있었을 텐데", "찰 수 있었을 텐데", "알 수 있었을 텐데", "거짓말 할 수 있었을 텐데", "들을 수 있었을 텐데", "살 수 있었을 텐데", "볼 수 있었을 텐데", "사랑할 수 있었을 텐데", "움직일 수 있었을 텐데", "열 수 있었을 텐데", "통과할 수 있었을 텐데", "지불할 수 있었을 텐데", "놀 수 있었을 텐데", "밀할 수 있었을 텐데", "읽을 수 있었을 텐데", "돌아갈 수 있었을 텐데", "달릴 수 있었을 텐데", "노래할 수 있었을 텐데", "앉을 수 있었을 텐데", "잘 수 있었을 텐데", "냄새날 수 있었을 텐데", "미소지을 수 있었을 텐데", "말할 수 있었을 텐데", "일어설 수 있었을 텐데", "시작할 수 있었을 텐데", "머무를 수 있었을 텐데", "멈출 수 있었을 텐데", "수영할 수 있었을 텐데", "말할 수 있었을 텐데", "이야기할 수 있었을 텐데", "여행할 수 있었을 텐데", "노력할 수 있었을 텐데", "돌릴 수 있었을 텐데", "방문할 수 있었을 텐데", "기다릴 수 있었을 텐데", "깰 수 있었을 텐데", "걸을 수 있었을 텐데", "씻을 수 있었을 텐데", "입을 수 있었을 텐데", "이길 수 있었을 텐데", "바랄 수 있었을 텐데", "일할 수 있었을 텐데", "걱정할 수 있었을 텐데", "쓸 수 있었을 텐데", "울릴 수 있었을 텐데", "가라앉을 수 있었을 텐데", "먹일 수 있었을 텐데", "쏠 수 있었을 텐데", "이끌 수 있었을 텐데", "기다릴 수 있었을 텐데", "바닥을 쓸 수 있었을 텐데", "울 수 있었을 텐데", "떠날 수 있었을 텐데", "구부릴 수 있었을 텐데", "잃을 수 있었을 텐데", "감을 수 있었을 텐데", "미끄러질 수 있었을 텐데", "팔 수 있었을 텐데", "빛날 수 있었을 텐데", "돌 수 있었을 텐데", "때릴 수 있었을 텐데", "깨울 수 있었을 텐데", "낳을 수 있었을 텐데", "찢을 수 있었을 텐데", "얼 수 있었을 텐데", "훔칠 수 있었을 텐데", "불 수 있었을 텐데", "던질 수 있었을 텐데", "탈 수 있었을 텐데", "오를 수 있었을 텐데", "흔들 수 있었을 텐데");
...
function function0301() {
  let kWord = ONE_SHOT_CONCEPT_K[02];
  let eWord = ONE_SHOT_CONCEPT_E[02] + " - " + ONE_SHOT_CONCEPT_2E[02];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
//function0302 형용사 용법
...
function function0401() {
  let kWord = ONE_SHOT_CONCEPT_K[03];
  let eWord = ONE_SHOT_CONCEPT_E[03] + " - " + ONE_SHOT_CONCEPT_2E[03];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
//0402 부사
...
function function0505ap() {
  ...
  document.getElementById('divRandom').innerText = eWord + " " + "- 몇 인칭? 단/복수?";
  document.getElementById('divRandom2').innerText = apWord;
}
//0505 PRONOUN_PERSONAL_SUBJECTIVE_ALL_K
function function0505ap1() {
  ...
  document.getElementById('divRandom').innerText = eWord + " " + "- 몇 인칭?";
  document.getElementById('divRandom2').innerText = apWord;
}
//0505 PRONOUN_PERSONAL_SUBJECTIVE_ALL_K
function function0505ap2() {
  ...
  document.getElementById('divRandom').innerText = eWord + " " + "- 단/복수?";
  document.getElementById('divRandom2').innerText = apWord;
}
...
function function0903() {
  ...
  let kWord = ARTICLE_THE_NUMERIC_K[valueNumber] + " = 단수 / 복수 ?";
...
function function1006() {
  ...
  let valueNumber2 = Math.floor(Math.random() * pronounBoxK.length);
  let kWord = pronounBoxK[valueNumber2] + TENSE_CONTINUOUS_K[valueNumber];
...
function function1006v2() {
  let valueNumber = Math.floor(Math.random() * TENSE_CONTINUOUS_K.length);
  let valueNumber2 = Math.floor(Math.random() * pronounBoxK.length);
  let kWord = pronounBoxK[valueNumber2] + "안 " + TENSE_CONTINUOUS_K[valueNumber];
...
//function1009 TENSE_CONTINUOUS_K 
...
function function1009v3() {
  let valueNumber = Math.floor(Math.random() * TENSE_CONTINUOUS_PAST_K.length);
  let valueNumber2 = Math.floor(Math.random() * pronounBoxK.length);
  let kWord = pronounBoxK[valueNumber2] + TENSE_CONTINUOUS_PAST_K[valueNumber];
...
function function1009v4() {
  let valueNumber = Math.floor(Math.random() * TENSE_CONTINUOUS_FUTURE_K.length);
  let valueNumber2 = Math.floor(Math.random() * pronounBoxK.length);
  let kWord = pronounBoxK[valueNumber2] + TENSE_CONTINUOUS_FUTURE_K[valueNumber];
...
function function1101a() {
  ...
  switch (chosenNumbers) {
    ...
    case 2:
      ...
      document.getElementById('divRandom').innerText = kWord;
      document.getElementById('divRandom2').innerText = eWord; break;
  }
}
...
function function1115() {
  let chosenNumbers = Math.floor(Math.random() * 18) + 1;
  console.log(chosenNumbers);
  switch (chosenNumbers) {
    case 1:
      function1114b();
...
//TODO 여기에 ing는 장소 넣고, p.p.는 by 사람 넣어서 경우의 수 만들기
//1302 PARTICIPLE_MODIFY_NOUN_K
function function1302() {
  let valueNumber1 = Math.floor(Math.random() * VERB_ALL_PRESENT_PARTICIPLE_K.length);
  ...
  switch (chosenNumbers) {
    case 1:
      kWord = VERB_ALL_PRESENT_PARTICIPLE_K[valueNumber1] + " " + noun3animalK[valueNumber3];
      eWord = VERB_ALL_PRESENT_PARTICIPLE_E[valueNumber1] + " " + noun3animalE[valueNumber3];
      document.getElementById("divRandom").innerText = kWord;
      document.getElementById("divRandom2").innerText = eWord;
      break;
    case 2:
      kWord = VERB_ALL_PAST_PARTICIPLE_K[valueNumber2] + " " + noun3animalK[valueNumber3];
      eWord = VERB_ALL_PAST_PARTICIPLE_E[valueNumber2] + " " + noun3animalE[valueNumber3];
      document.getElementById("divRandom").innerText = kWord;
      document.getElementById("divRandom2").innerText = eWord;
      break;
  }
...
function function1309v2() {
  ...
  switch (chosenNumbers) {
    case 1:
      kWord = noun3animalK[valueNumber3] + "(이/가) " + VERB_ALL_PRESENT_PARTICIPLE_K[valueNumber1] + " 채로";
      eWord = "with " + noun3animalE[valueNumber3] + " " + VERB_ALL_PRESENT_PARTICIPLE_E[valueNumber1];
      document.getElementById("divRandom").innerText = kWord;
      document.getElementById("divRandom2").innerText = eWord;
      break;
    case 2:
      kWord = noun3animalK[valueNumber3] + "(이/가) " + VERB_ALL_PAST_PARTICIPLE_K[valueNumber2] + " 채로";
      eWord = "with " + noun3animalE[valueNumber3] + " " + VERB_ALL_PAST_PARTICIPLE_E[valueNumber2];
      document.getElementById("divRandom").innerText = kWord;
      document.getElementById("divRandom2").innerText = eWord;
      break;
  }
...
//TODO 고쳐야함
//function1402a PASSIVE_SENTENCE_K 미완성
function function1402a() {
  let valueNumber = Math.floor(Math.random() * PASSIVE_SENTENCE_2K.length);
  let kWord = PASSIVE_SENTENCE_2K[valueNumber];
  let eWord = PARTICIPLE_WITH_E[valueNumber];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
...
function function1604a() {
  ...
  switch (chooseNumbers) {
    case 1:
      ...
      eWord = THINGS_S_Be_E[valueNumber1] + " " + ADJECTIVE_SUPERATIVE_POSITIVE_THINGS_E[valueNumber11];
      document.getElementById("divRandom").innerText = kWord;
      document.getElementById("divRandom2").innerText = eWord;
      break;
    case 2:
      ...
      eWord = THINGS_NoS_Be_E[valueNumber2] + " " + ADJECTIVE_SUPERATIVE_POSITIVE_THINGS_E[valueNumber22];
      document.getElementById("divRandom").innerText = kWord;
      document.getElementById("divRandom2").innerText = eWord;
      break;
    case 3:
      ...
      eWord = PEOPLE_S_Be_E[valueNumber3] + " " + ADJECTIVE_SUPERATIVE_POSITIVE_PEOPLE_E[valueNumber33];
      document.getElementById("divRandom").innerText = kWord;
      document.getElementById("divRandom2").innerText = eWord;
      break;
    case 4:
      ...
      eWord = PEOPLE_NoS_Be_E[valueNumber4] + " " + ADJECTIVE_SUPERATIVE_POSITIVE_PEOPLE_E[valueNumber44];
      document.getElementById("divRandom").innerText = kWord;
      document.getElementById("divRandom2").innerText = eWord;
      break;
  }
...
function function1802b() {
  ...
  let kWord = "만약 " + PRONOUN_PERSONAL_SUBJECTIVE_2K[valueNumber2] + " " + VERB_GENERAL_POSITIVE_ALL[valueNumber1] + "면" + ", \n" + PRONOUN_PERSONAL_SUBJECTIVE_K[valueNumber3] + " " + SUBJUNCTIVE_AUXILIARY_WOULD_K[valueNumber];
  // let eWord = "If " + "S" + "과거동사" + "," + "S" + "would should could might" + VERB_SENTENCE_FORM_1_E[valueNumber];
  let eWord = "If " + PRONOUN_PERSONAL_SUBJECTIVE_SMALL_E[valueNumber2] + " " + VERB_GENERAL_PAST_ALL_E[valueNumber1] + ", \n" + PRONOUN_PERSONAL_SUBJECTIVE_SMALL_E[valueNumber3] + " would " + VERB_SENTENCE_FORM_1_E[valueNumber];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
...
function function1802c() {
  ...
  let valueNumber2 = Math.floor(Math.random() * PRONOUN_PERSONAL_SUBJECTIVE_2K.length);
  let valueNumber3 = Math.floor(Math.random() * PRONOUN_PERSONAL_SUBJECTIVE_2K.length);
  let kWord = "만약 " + PRONOUN_PERSONAL_SUBJECTIVE_2K[valueNumber2] + " " + VERB_GENERAL_POSITIVE_ALL[valueNumber1] + "면" + ", \n" + PRONOUN_PERSONAL_SUBJECTIVE_K[valueNumber3] + " " + SUBJUNCTIVE_AUXILIARY_SHOULD_K[valueNumber];
  let eWord = "If " + PRONOUN_PERSONAL_SUBJECTIVE_SMALL_E[valueNumber2] + " " + VERB_GENERAL_PAST_ALL_E[valueNumber1] + ", \n" + PRONOUN_PERSONAL_SUBJECTIVE_SMALL_E[valueNumber3] + " should " + VERB_SENTENCE_FORM_1_E[valueNumber];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
...
function function1802d() {
  ...
  let valueNumber2 = Math.floor(Math.random() * PRONOUN_PERSONAL_SUBJECTIVE_2K.length);
  let valueNumber3 = Math.floor(Math.random() * PRONOUN_PERSONAL_SUBJECTIVE_2K.length);
  let kWord = "만약 " + PRONOUN_PERSONAL_SUBJECTIVE_2K[valueNumber2] + " " + VERB_GENERAL_POSITIVE_ALL[valueNumber1] + "면" + ", \n" + PRONOUN_PERSONAL_SUBJECTIVE_K[valueNumber3] + " " + SUBJUNCTIVE_AUXILIARY_COULD_K[valueNumber];
  // let eWord = "If " + "S" + "과거동사" + "," + "S" + "would should could might" + VERB_SENTENCE_FORM_1_E[valueNumber];
  let eWord = "If " + PRONOUN_PERSONAL_SUBJECTIVE_SMALL_E[valueNumber2] + " " + VERB_GENERAL_PAST_ALL_E[valueNumber1] + ", \n" + PRONOUN_PERSONAL_SUBJECTIVE_SMALL_E[valueNumber3] + " could " + VERB_SENTENCE_FORM_1_E[valueNumber];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
...
function function1802e() {
  ...
  let valueNumber3 = Math.floor(Math.random() * PRONOUN_PERSONAL_SUBJECTIVE_2K.length);
  // let kWord = "만약 " + "S" + "현재동사 뜻이 라면" + "," + "S" + "일텐데";
  let kWord = "만약 " + PRONOUN_PERSONAL_SUBJECTIVE_2K[valueNumber2] + " " + VERB_GENERAL_POSITIVE_ALL[valueNumber1] + "면" + ", \n" + PRONOUN_PERSONAL_SUBJECTIVE_K[valueNumber3] + " 아마도 " + SUBJUNCTIVE_AUXILIARY_MIGHT_K[valueNumber];
  // let eWord = "If " + "S" + "과거동사" + "," + "S" + "would should could might" + VERB_SENTENCE_FORM_1_E[valueNumber];
  let eWord = "If " + PRONOUN_PERSONAL_SUBJECTIVE_SMALL_E[valueNumber2] + " " + VERB_GENERAL_PAST_ALL_E[valueNumber1] + ", \n" + PRONOUN_PERSONAL_SUBJECTIVE_SMALL_E[valueNumber3] + " might " + VERB_SENTENCE_FORM_1_E[valueNumber];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
...
function function1901() {
  let kWord = ONE_SHOT_CONCEPT_K[18];
  let eWord = ONE_SHOT_CONCEPT_E[18] + " - " + ONE_SHOT_CONCEPT_2E[18];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
...
function function1901a() {
  ...
  let eWord = INTERROGATIVE_K[valueNumber];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
...
//function2404v3 SENTENCE_THERE_BE_K
...
function function2601() {
  let kWord = ONE_SHOT_CONCEPT_K[25];
  let eWord = ONE_SHOT_CONCEPT_E[25] + " - " + ONE_SHOT_CONCEPT_2E[25];
  document.getElementById("divRandom").innerText = kWord;
  document.getElementById("divRandom2").innerText = eWord;
}
